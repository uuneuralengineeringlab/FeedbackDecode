function [subX, subZ, subK, subT, subKDFTimes, TrialStruct] = parseTrainingData_RandTrials_COB(KefFname,KdfFname,varargin)
...
RandTrials = isRandTrials(TrialStruct);
if RandTrials
    disp('random trials detected...')
else
    disp('sequential trials detected...')
end
...
if ~isempty(TrialStruct)
    for k = 1:length(TrialStruct)
        ...
        if RandTrials
            skipflag = ~isempty(curMvntIdx); %use all trials for random trials
        else
            skipflag = ~isempty(curMvntIdx) && ~any(TrialStruct(k).MvntMat(:) ~= lastMvnt(:)); %skip 1st trial for sequential trials
        end
        
        if skipflag %look for desired movements, and throw out first trial
