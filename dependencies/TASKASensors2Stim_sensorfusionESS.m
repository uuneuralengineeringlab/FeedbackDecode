function [Freq, Amp, varargout] = TASKASensors2Stim_sensorfusionESS(TASKASensorsIR, TASKASensorsBAR,TASKASensorLabels, IRSensorPulse, StimCell,varargin)
...
% test values
% TASKASensorsIR = [0, 17590, 18050, 14170]'+1000;
% TASKASensorsBAR = [0, 85500, 85628, 85320]'+[0; 100; 100; 500];



TASKASensors = [TASKASensorsIR(2:4) TASKASensorsBAR(2:4)];
TASKASensors = TASKASensors - [17525,86000;17960,86050;14125,85760];
TASKASensors(TASKASensors<0) = 0;
TASKASensors = TASKASensors./[1000,200; 350,200; 1500,300];
TASKASensors(TASKASensors>1) = 1;


for i = 1:3
    if TASKASensors(i,2) > .2
        if TASKASensors(i,2) > .6
            TASKASensors(i,1) = .5 + TASKASensors(i,2) * .5;
        else
            TASKASensors(i,1) = 0;
        end
      %TASKASensors(i,1) > .15
    elseif TASKASensors(i,1) > .15
        TASKASensors(i,1) = TASKASensors(i,1)*.3 * .5 + TASKASensors(i,1)*.3 * .5 * sin(IRSensorPulse*6.28/33);
    else
        TASKASensors(i,1) = 0;
%     else
%         TASKASensors(i,1) = TASKASensors(i,1)*.4;
    end
end
TASKASensors = reshape(TASKASensors,[6,1]);
% if IdxContact(1) == 1
%     
% for i = 1:3
%     if TASKASensors(i,2) > .15
%         TASKASensors(i,1) = .4 + TASKASensors(i,2) * .6;
% %     else TASKASensors(i,1)
% %         TASKASensors(i,1) = TASKASensors(i,1)*.3 * .5 + TASKASensors(i,1)*.3 * .5 * sin(IRSensorPulse*6.28/33);
% %     else
% %         TASKASensors(i,1) = TASKASensors(i,1)*.4;
%     end
