function [PARAM,x,z]=NonLinSVReg_train(x,z)
% NonLinSVReg_train - calculate linear SVR training data
%
%   [PARAM,x,z]=NonLinSVReg_train(x,z)
%
%   Inputs
%
...
%                   particular, this algorithm calcualtes PARAM.B which is
%                   the least squares regression coefficients, PARAM.N
...
%                   training, and PARAM.nDelay which is the number of delay
...
kernelType = '2'; %Radial basis function (Gaussian) kernel (exp(-gamma*|u-v|^2))
...
for n = 1:nKinematics
    ...
    ['-s ' svmType ' -t ' kernelType ' -h 1 ' ' -d '  degree ' -p ' epsilon ' -q']);% ' -q']);
