function [cutaneousMap,proprioceptionMap,amplitudeMap, durationMap, frequencyMap1, frequencyMap2, frequencyMap3, varargout] = calibrateEncode(SRF)
...
%------------- BEGIN CODE ---------------
    ...
    for jj = 1:channelCount
        channelSRF = getSRF(SRF,'electrodes_d',jj,'onlyinvolves');
...
%------------- BEGIN CODE ---------------
    try
        ...
        SRF = assignProprioceptionSensor(SRF);
        sensorLabels = SRF.info.proprioceptionLabels;
    end
    try %get num channels
        channelCount = SRF.info.numberOfChannels_d;
    ...
    for ii = 1:numSensor
        ...
        sensorSRF = getSRF(SRF,'proprioceptionSensor_s',sensorName,'onlyinvolves');
        if(~isempty(sensorSRF.output))
            for jj = 1:channelCount
                channelSRF = getSRF(sensorSRF,'electrodes_d',jj,'onlyinvolves');
                if(~isempty(channelSRF.output))
...
%------------- BEGIN CODE ---------------
    ...
    for ii = 1:numSensor %create map
        sensorName = sensorNames{ii};
        sensorSRF = getSRF(SRF,'contactSensor_s',sensorName,'onlyinvolves');
        if(~isempty(sensorSRF.output))
            for jj = 1:channelCount
                channelSRF = getSRF(sensorSRF,'electrodes_d',jj,'onlyinvolves');
                if(~isempty(channelSRF.output))
