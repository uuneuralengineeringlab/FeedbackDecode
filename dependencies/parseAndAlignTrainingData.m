function [subX, subZ, subK, subT, subKDFTimes, TrialStruct] = parseAndAlignTrainingData(KefFname,KdfFname,varargin)
        ...
        if ~isempty(curMvntIdx) && ~any(TrialStruct(k).MvntMat(:) ~= lastMvnt(:)) %look for desired movements, and throw out first trial
            ...
            if ismember(curMvntIdx,KalmanMvnts)
                ...
                if (curMvntSgn &&  KalmanGain(KalmanMvnts==curMvntIdx,1))
                    ...
                    % align kinematics and data
                    Lag = (-12:12);
                    CMean = zeros(length(Lag),1);
                    curZ = Z(SelectedInd, curKDFInd);
                    curX = X(KalmanMvnts, curKDFInd);
                    for j=1:length(Lag)
                        ZShift = circshift(curZ,Lag(j),2);
                        C = (curX*ZShift')./bsxfun(@(x,y)x*y,sqrt(sum(curX.^2,2)),sqrt(sum(curZ.^2,2))');
                        CMean(j) = nanmean(abs(C(:)));
                    end
                    [~,idx] = max(CMean);
                    MaxLag = Lag(idx);
                    ZShift = circshift(Z(SelectedInd, curKDFInd),MaxLag,2);

                    subX(:,curSubInd(1):curSubInd(2)) = X(KalmanMvnts, curKDFInd);
                    subZ(:,curSubInd(1):curSubInd(2)) = ZShift;
