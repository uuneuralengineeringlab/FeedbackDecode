...
if SS.TASKA.Ready
    try
        ...
        if SS.LCWrist_Ready
            SS.LCWrist_history = [SS.LCWrist_history(:,2:20) [-CurrX(10); -CurrX(12)]]; %% 1st row is one DOF 2nd row is second DOF
            updateTaskaWrist(SS.LCWrist, mean(SS.LCWrist_history,2)); %% Takes mean rowise
    ...
    for k=1:length(SS.StimChan)
        switch SS.StimMode %Off, Manual, MSMS, VRE
            ...
            case '3DHand' %switched 08182021 ESS to workaround a labview issue             
                if SS.IRSensorPulse == 33
                    SS.IRSensorPulse = 1;
                else
                    SS.IRSensorPulse = SS.IRSensorPulse + 1;
                end
                [SS.ContStimFreq(k), SS.ContStimAmp(k)] = TASKASensors2Stim_sensorfusionESS(SS.TASKASensors.IR, SS.TASKASensors.baro, SS.TASKASensorLabels, SS.IRSensorPulse, SS.StimCell(k,:),SS.DEKA.SensorThresholds');
                if k == 2
                    SS.TASKASensors.IR
