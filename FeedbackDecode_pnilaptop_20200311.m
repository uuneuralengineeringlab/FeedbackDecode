    ...
    try
        ...
        switch LVCell{1}
            ...
            case 'EndTrial' %acquiring endtrial event
                ...
                if SS.EEGTrigger && (strcmp(SS.KinSrc, 'Decode'))  % MDP 20190813 MP: need to update... and add cases for other cog load items
                    % send stop trigger
                    disp('Stop EEG Trigger')
                    xippmex_20180417('digout',5,1) % parallel 1: targOff
                end
...
%                 SS.DEKA.BLSensors = calibrateDEKA_wristp(); % fixed for wrists in pos mode (dk 2018-03-16)
            ...
            case 'TargOn' %acquiring targon event during training
                ...
                if SS.EEGTrigger && (strcmp(SS.KinSrc, 'Decode')) % MDP 20190813
                    ...
                    xippmex_20180417('digout',5,2) % parallel 2: targOn
                end
                if SS.SecondaryTask
                    if ~SS.CogLoadStimOn
                ...
                if SS.RecordECG
                    ...
                    switch compName
                        ...
                        case 'BIOEN-LAPTOP'
                            ecgport = '10';
                        otherwise
                            warning('ECG Port not set for this computer')
                    end %switch
                    [shimmerECG, ~] = ecgconnect(ecgport);
                    ecgStartRecordTS = ecgstart(shimmerECG);
                else
                    if exist('shimmer3')
                        ecgStopRecordTS = ecgstop(shimmerECG);
                        ecgdisconnect(shimmerECG);
                    end
                end
        end %switch
    catch ME
...
if ~isempty(SS.DigIO_TS)
    for i=1:length(SS.DigIO_TS)
        if (SS.DigEvents(i).reason == 35) && (SS.DigEvents(i).sma1 == 32767) % parallel also equals 4
            fprintf(SS.CogLoadFID,'StimulusOn,NIPTime=%0.0f\r\n',SS.DigIO_TS(i) - SS.RecStart);
        elseif (SS.DigEvents(i).reason == 35) && (SS.DigEvents(i).sma1 == 0) % parallel also equals 3
            fprintf(SS.CogLoadFID,'StimulusOff,NIPTime=%0.0f\r\n',SS.DigIO_TS(i) - SS.RecStart);
        elseif (SS.DigEvents(i).reason == 33) && (SS.DigEvents(i).parallel == 2)
            fprintf(SS.CogLoadFID,'EEGTriggerOn,NIPTime=%0.0f\r\n',SS.DigIO_TS(i) - SS.RecStart);
        elseif (SS.DigEvents(i).reason == 33) && (SS.DigEvents(i).parallel == 1)
            fprintf(SS.CogLoadFID,'EEGTriggerOff,NIPTime=%0.0f\r\n',SS.DigIO_TS(i) - SS.RecStart);
        elseif (SS.DigEvents(i).reason == 2) && (SS.DigEvents(i).sma1 == 32767)
            if SS.CogLoadStimOn
                SS.CogLoadButtonPress = 1;
            end
            fprintf(SS.CogLoadFID,'ButtonPress,NIPTime=%0.0f\r\n',SS.DigIO_TS(i) - SS.RecStart);
        elseif (SS.DigEvents(i).reason == 2) && (SS.DigEvents(i).sma1 == 0)
            fprintf(SS.CogLoadFID,'ButtonRelease,NIPTime=%0.0f\r\n',SS.DigIO_TS(i) - SS.RecStart);
        end
    end
end
    ...
    if SS.SecondaryTask % if secondary task button pressed in LV
        if ~SS.CogLoadStimOn % if not buzzing
            if SS.CurrTS > SS.CogLoadNextStimTS
                % start next stimulus
                SS.CogLoadStimOn = 1;
                xippmex_20180417('digout',[1, 5],[1, 4]); % parallel port: 4 stim on, 3 off
            ...
            if ((SS.CurrTS > SS.CogLoadNextStimOffTS) || ... % turn off buzz if longer than stim duration
                ...
                SS.CogLoadButtonPress = 0;
                SS.CogLoadStimOn = 0;
                xippmex_20180417('digout',[1,5],[0,3]);
            end
        end
    end
