...
try
    SS = t.UserData;
    SS.MCalcTic = double(xippmex_20180417('time'));
    SS = acqEvents(SS);
    if SS.Stop
        stop(t);
    end
    ...
    SS = saveTask(SS);
    SS = savePHand(SS);
    SS.MCalcTime = (double(xippmex_20180417('time'))-SS.MCalcTic)/30000; %calculation time within loop
...
SS = t.UserData;
SS.RecEnd = double(xippmex_20180417('time')); %smw - get end NIP time for Synching purposes
SS = orderfields(SS);
assignin('base','SS',SS)
xippmex_20180417('stim','enable',0); pause(1);
try
%     xippmex_20180417('trial',SS.XippOpers,'stopped'); pause(1);
    xippmex_20180417('trial','stopped'); pause(1);
catch
    disp('xippmex call to stop recording crashed...')
end
xippmex_20180417('close'); clear('xippmex_20180417'); mj_close;
...
if strcmp(LabviewIP,'127.0.0.1')
    ...
    [~,compstr] = system('power shell get-wmiobject -class win32_computersystem');
...
pause(1)
try
    SS.XippTS = double(xippmex_20180417('time'));
...
%         xippmex_20180417('trial',SS.XippOpers,'recording',fullfile(SS.FullDataFolder,[SS.DataFolder,'-']));
        xippmex_20180417('trial','recording',fullfile(SS.FullDataFolder,[SS.DataFolder,'-']));
...
SS.DNeural = zeros(SS.NumNeuralIdxs,SS.DLNeuralMax);
 if ~isempty(SS.AvailNeural)
    [DNeural,DNeuralTS] = xippmex_20180417('cont',SS.AvailNeural,SS.DLNeuralMaxMS,'raw'); SS.CurrTS = double(DNeuralTS); %slow (1.08)
    if ~isempty(DNeural)
        SS.DNeural(SS.AvailNeuralIdx,:) = DNeural; %slow (0.23)
    end
else
    SS.CurrTS = double(xippmex_20180417('time'))-SS.DLNeuralMax;
end
...
% if ~isempty(SS.AvailNeural)
%     [~,ApCell,ApWfCell] = xippmex_20180417('spike',SS.AvailNeural,0); %slow (1.78)
%     [~,StCell,StWfCell] = xippmex_20180417('spike',SS.AvailNeural,1); %slow (1.57)
...
if ~isempty(SS.AvailEMG)
    DEMG = xippmex_20180417('cont',SS.AvailEMG,SS.DLNeuralMaxMS,'lfp',SS.CurrTS);
    if ~isempty(DEMG)
...
[~,SS.DigIO_TS,SS.DigEvents] = xippmex_20180417('digin');
if ~isempty(SS.DigEvents)
    if any([SS.DigEvents.reason]==4) %sma2 -- digital I/O input 2
        xippmex_20180417('stim','enable',0);
        SS.StimMode = 'Off';
        ...
        switch LVCell{1}
            ...
            case 'EndTrial' %acquiring endtrial event
                ...
                if SS.EEGTrigger && (strcmp(SS.KinSrc, 'Decode'))
                    ...
                    xippmex_20180417('digout',5,targ2EEGEvent(SS.TargRad, SS.T, 'TargOff'))
                end
                ...
                for k=1:length(SS.AvailStimHS)
                    ...
                    if ~isempty(StimIdx)
                        if StimIdx>=1 && StimIdx<=6
                            xippmex_20180417('stim','res',SS.AvailStimHS(k),StimIdx)
                        end
                    end
                end
...
%                 [SS.TASKA.Obj, SS.TASKA.Ready] = openTASKA();
            ...
            case 'TargOn' %acquiring targon event during training
                ...
                if SS.EEGTrigger && (strcmp(SS.KinSrc, 'Decode')) % MDP 20190813
                    ...
                    xippmex_20180417('digout',5,targ2EEGEvent(SS.TargRad, SS.T, 'TargOn'))
                end
                if SS.SecondaryTask
                    if ~SS.CogLoadStimOn
...
%                         Do this is AUX  nb bxl_build('runK')
            ...
            case 'ResetTrial'
                ...
                disp({'Beta Values:' num2str(SS.betaValues)})
            case 'ConnectECG'
                if SS.ConnectECG % connect shimmer ECG device and start recording
                    [SS.shimmerECG, ~] = ecgconnect();
                else % stop recording and disconnect
                    if isfield(SS,'shimmerECG')
                        ecgdisconnect(SS.shimmerECG);
                    end
                end
...
%     SS.ContDEKAMotors = SS.ContLV(44:51);
    ...
    if ~isempty(SS.StimChan)        
        [~,~,SS.StimWfCell] = xippmex_20180417('spike',SS.StimChan(1),1);
...
%         pos = [SS.XHat(2),SS.XHat(3),SS.XHat(4),SS.XHat(5),SS.XHat(1),-SS.XHat(6)];
        ...
        if SS.TASKA.Count >= SS.TASKA.CountMax  % Updating every cycle causes lag <--  % Commented out 6/17/19 TCH
            updateTASKA(SS.TASKA.Obj,pos',SS.TASKA.RestPositions);
            SS.TASKA.Count = 0; % Commented out 6/17/19 TCH
            SS.TASKAMotors = pos;
        else  % Commented out 6/17/19 TCH
...
if ~isempty(SS.StimChan)
    ...
    SS.CurrTime = xippmex_20180417('time');
    ...
    if any(SS.StimIdx)
        try
            xippmex_20180417('stimseq',SS.StimSeq(SS.StimIdx));
        catch ME
            if isempty(ME.stack)
                fprintf('message: %s\r\n',ME.message);
            else
...
for k=1:length(SS.AvailStimHS)
    ...
    if ~isempty(StimIdx)
        if StimIdx>=1 && StimIdx<=6
            xippmex_20180417('stim','enable',0);
            xippmex_20180417('stim','res',SS.AvailStimHS(k),StimIdx)
            xippmex_20180417('stim','enable',1);
...
if ~isempty(SS.DigIO_TS)
    for i=1:length(SS.DigIO_TS)
        ...
        switch cogLoadEventType % modified 20200317 - would include SS.TargRad for size but doesn't update correctly...
            case 'BuzzOn'
                fprintf(SS.CogLoadFID,'BuzzOn,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
            case 'BuzzOff'
                fprintf(SS.CogLoadFID,'BuzzOff,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
            case 'TargOn'
                fprintf(SS.CogLoadFID,'TargOn,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
            case 'TargOff'
                fprintf(SS.CogLoadFID,'TargOff,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
            case 'ButtonPressEEGTrigger'
                fprintf(SS.CogLoadFID,'ButtonPressEEGTrigger,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
            case 'ButtonReleaseEEGTrigger'
                fprintf(SS.CogLoadFID,'ButtonReleaseEEGTrigger,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
            case 'ButtonPressRawNIP'
                if SS.CogLoadStimOn
                    SS.CogLoadButtonPress = 1;
                end
                fprintf(SS.CogLoadFID,'ButtonPressRawNIP,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
                xippmex_20180417('digout',[1, 5],[0, targ2EEGEvent(SS.TargRad, SS.T, 'ButtonPress')]);
            case 'ButtonReleaseRawNIP'
                fprintf(SS.CogLoadFID,'ButtonReleaseRawNIP,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
                xippmex_20180417('digout',[1, 5],[0, targ2EEGEvent(SS.TargRad, SS.T, 'ButtonRelease')]);
            case 'Unknown'
                fprintf(SS.CogLoadFID,'UnknownEvent,NIPTime=%0.0f,ParallelID=%0.0f\r\n', ...
                    [SS.DigIO_TS(i) - SS.RecStart, double(SS.DigEvents(i).parallel)]);
        end
    end
end
    ...
    if SS.SecondaryTask % if secondary task button pressed in LV
        if ~SS.CogLoadStimOn % if not buzzing
            if SS.CurrTS > SS.CogLoadNextStimTS
                % start next stimulus
                SS.CogLoadStimOn = 1;
                xippmex_20180417('digout',[1, 5],[1, targ2EEGEvent(SS.TargRad, SS.T, 'BuzzOn')]); 
            ...
            if ((SS.CurrTS > SS.CogLoadNextStimOffTS) || ... % turn off buzz if longer than stim duration
                ...
                SS.CogLoadButtonPress = 0;
                SS.CogLoadStimOn = 0;
                xippmex_20180417('digout',[1,5],[0,targ2EEGEvent(SS.TargRad, SS.T, 'BuzzOff')]);
            end
        end
    end
...
while 1
    try
        if xippmex_20180417('tcp')  %% MB 20200302 original is just xippmex_20180417 for UDP 
        else 
            xippmex_20180417('close'); clear('xippmex_20180417');
            disp('Unable to initialize TCP xippmex');
            xippmex_20180417();
            disp('using UDP mode');
        end
%             SS.XippOpers = xippmex_20180417('opers'); pause(0.1);
            SS.AvailChanList = xippmex_20180417('elec','all'); pause(0.1);
            SS.AvailStimList = xippmex_20180417('elec','stim'); pause(0.1);            
...
if ~isempty(SS.AvailNeural)
    xippmex_20180417('signal',SS.AvailNeuralHS,'raw',ones(length(SS.AvailNeuralHS),1)); pause(0.1); %only 1st headstage channels need to be sent
    xippmex_20180417('signal',SS.AvailNeuralHS,'lfp',zeros(length(SS.AvailNeuralHS),1)); pause(0.1);
    xippmex_20180417('signal',SS.AvailNeural,'spk',ones(length(SS.AvailNeural),1)); pause(0.1); %all available channels need to be sent to xippmex
    for k=1:length(SS.AvailNeuralHS)
        xippmex_20180417('filter','set',SS.AvailNeuralHS(k),'spike',3); pause(0.1);
        xippmex_20180417('fastsettle','stim',SS.AvailNeuralHS(k),3,1); %set to same front port
    end
end

% Enabling appropriate emg channels
if ~isempty(SS.AvailEMG)
    xippmex_20180417('signal',SS.AvailEMGHS,'raw',ones(length(SS.AvailEMGHS),1)); pause(0.1);
    xippmex_20180417('signal',SS.AvailEMGHS,'lfp',ones(length(SS.AvailEMGHS),1)); pause(0.1);
    xippmex_20180417('signal',SS.AvailEMG,'spk',zeros(length(SS.AvailEMG),1)); pause(0.1);
    for k=1:length(SS.AvailEMGHS)
        xippmex_20180417('filter','set',SS.AvailEMGHS(k),'lfp',4); pause(0.1); %bandpass "EMG" 15-350 to start
        xippmex_20180417('filter','set',SS.AvailEMGHS(k),'lfp notch',3); pause(0.1); %notch to start
    end
end

% Enabling appropriate stim channels
if ~isempty(SS.AvailStim)
    xippmex_20180417('signal',SS.AvailStim,'stim',ones(length(SS.AvailStim),1)); pause(0.1);
end

% Enabling appropriate analog channels
if ~isempty(SS.AvailAnalog)
    xippmex_20180417('signal',SS.AvailAnalog,'1ksps',ones(length(SS.AvailAnalog),1)); pause(0.1);
    xippmex_20180417('signal',SS.AvailAnalog,'30ksps',zeros(length(SS.AvailAnalog),1)); pause(0.1);
end

% Disabling unwanted emg channels
% if ~isempty(SS.DisableEMG)
%     xippmex_20180417('signal',SS.DisableEMGHS,'raw',zeros(length(SS.DisableEMGHS),1)); pause(0.1);
%     xippmex_20180417('signal',SS.DisableEMGHS,'lfp',zeros(length(SS.DisableEMGHS),1)); pause(0.1);
%     xippmex_20180417('signal',SS.DisableEMG,'spk',zeros(length(SS.DisableEMG),1)); pause(0.1);
% end

SS.SfTable = [0.125,0.25,0.5]; %uV/bit returned from xippmex call: output = xippmex_20180417('adc2phys', 1);
SS.SfNeural = 0.25;
if ~isempty(SS.AvailNeuralHS)
    SS.SfNeural = SS.SfTable(xippmex_20180417('adc2phys',SS.AvailNeuralHS));
...
if ~isempty(SS.AvailEMGHS)
    SS.SfEMG = SS.SfTable(xippmex_20180417('adc2phys',SS.AvailEMGHS));
